...
#   --keep-oncall-dirs      保留存量 oncallChats 里的老业务目录（默认会重写成角色目录，见下）
...
#     记忆串味 —— 而那正是读隔离要防的东西）
#
# ⚠️ 存量的**显式 oncall 绑定**（oncallChats）会架空角色系统：
#   resolvePinnedWorkingDir 的优先级是 oncallEntry.workingDir > defaultWorkingDir —— 只要某个群
#   有显式绑定，pin 就取绑定里的目录，**角色目录压根不查**。老机器上这些绑定往往指向业务代码目录
#   （如 ~/kgp、~/fi），那里没有 .botmux-dir.json → 这些群里角色系统不进场、卡片脚注没有角色名、
#   也切不了角色。
#   所以默认把 oncallChats 里每条绑定的 workingDir **重写成角色目录**。
#   ⚠️ 这会改变行为：这些群里 bot 的默认 cwd 不再是业务目录。想保留就加 --keep-oncall-dirs。
#
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --keep-oncall-dirs) KEEP_ONCALL=1; shift ;;
...
node -e '
const fs=require("fs");
const [p,excludeRaw,owner,planOut,keepOncall]=process.argv.slice(1);
...
for (const b of bots) {
  ...
  if (staleOncall.length && keepOncall !== "1") {
    ...
    for (const c of staleOncall) changes.push(`     · ${c.chatId} : ${c.workingDir} → ${roleDir}`);
  } else if (staleOncall.length) {
    console.log(`       ⓘ ${staleOncall.length} 条 oncall 绑定保留老目录（--keep-oncall-dirs）——这些群里角色系统不生效`);
  }
...
' "$BOTS_JSON" "$EXCLUDE" "$OWNER" "$PLAN" "$KEEP_ONCALL"
...
node -e '
const fs=require("fs");
const [p,targetsRaw,owner,resolvedPath,keepOncall]=process.argv.slice(1);
...
for (const b of bots) {
  ...
  if (keepOncall !== "1" && Array.isArray(b.oncallChats)) {
    for (const c of b.oncallChats) {
      if (c && c.workingDir !== roleDir) {
        console.log(`    oncall ${c.chatId}: ${c.workingDir} → ${roleDir}`);
        c.workingDir = roleDir;
      }
    }
  }
...
' "$BOTS_JSON" "$TARGETS" "$OWNER" "$RESOLVED" "$KEEP_ONCALL"
...
node -e '
const fs=require("fs");
const [p,targetsRaw,keepOncall]=process.argv.slice(1);
...
for (const id of targets) {
  ...
  const checks=[
    ...
    ["oncall 绑定全部指向角色目录（否则那些群里角色系统不生效）",
      keepOncall === "1" || (b.oncallChats||[]).every(c => c.workingDir === b.defaultWorkingDir)],
