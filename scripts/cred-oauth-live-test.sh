...
ALERT_APP=cli_aacd7ceeb5789cc6
ALERT_TO=ou_e0be3737b53350f5c529e6cfb5227157
NODE=/Users/ksher/.local/node-v24/bin/node
...
ALERT_APP=$ALERT_APP ALERT_TO=$ALERT_TO FORCE=1 SEED=1 SUSPEND=0 \
  bash "$HOME/beta/botmux/scripts/bot-cred-refresh-oauth.sh"
RC=$?
echo "refresh rc=$RC"
echo "[after]"; fps
...
OUT=$(CLAUDE_CONFIG_DIR="$HOME/.claude" perl -e 'alarm 90; exec @ARGV' /Users/ksher/.local/bin/claude -p "reply with exactly: OK" </dev/null 2>&1)
CRC=$?
