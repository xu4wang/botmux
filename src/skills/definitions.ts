...
本 skill 只帮助检查、修复、校验和迁移已有 v2 workflow JSON；旧资产的兼容运行入口是 \`botmux template ...\` / \`/template ...\`，仅保留一个迁移周期。
...
\`\`\`bash
botmux template run <workflowId> --param key=value
# 或在飞书话题里:
...
保存是用户显式动作，不自动保存每个 run；成功 run 之后可以简短建议一次“要不要保存复用”，不要重复推销。v2 的 \`botmux template ...\` / \`/template ...\` 只用于存量迁移，新工作一律留在本 skill。
