...
export function buildArchitectGoal(specPath: string, specJsonPath: string): string {
  ...
  return [
    ...
    '- botmux-schedule input: `{ name:<string>, schedule:<string>, prompt:<string>, workingDir:<string>, larkAppId:{"$ref":"context.larkAppId"}, chatId:{"$ref":"context.chatId"}, chatType:{"$ref":"context.chatType"}, rootMessageId?:{"$ref":"context.rootMessageId"}, scope?:"thread"|"chat", executionPosition?:"top-level"|"topic"|"new-topic", topicTitle?:<string>, repeat?:{times:number|null} }`. OMIT `parsed`: runtime derives and freezes it once per run. `topic` requires rootMessageId; `new-topic` creates a fresh topic each run and may use topicTitle as its seed/title; only `top-level` consults ordinary-group reply mode.',
